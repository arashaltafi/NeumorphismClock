package com.arash.altafi.neumorphismclock.utils
...
class NeumorphismClock(
    ...
    private var iconSize = TypedValue.applyDimension(
    ...
    // Declare colors
    private var lightBackgroundColor =
        ContextCompat.getColor(context, R.color.defaultLightBackgroundColor)
    ...
    private var minHourHandsColor =
        ContextCompat.getColor(context, R.color.defaultMinHourHandsColor)
    ...
    private fun setUpAttributes(attributes: AttributeSet?) {
        val typedArray =
            context.theme.obtainStyledAttributes(attributes, R.styleable.NeumorphismClock, 0, 0)

        lightBackgroundColor = ContextCompat.getColor(
            context,
            typedArray.getResourceId(
                R.styleable.NeumorphismClock_lightBackgroundColor,
                R.color.defaultLightBackgroundColor
            )
        )
        lightShadowColor = ContextCompat.getColor(
            context,
            typedArray.getResourceId(
                R.styleable.NeumorphismClock_lightShadowColor,
                R.color.defaultLightShadowColor
            )
        )
        darkShadowColor = ContextCompat.getColor(
            context,
            typedArray.getResourceId(
                R.styleable.NeumorphismClock_darkShadowColor,
                R.color.defaultDarkShadowColor
            )
        )
        borderColor = ContextCompat.getColor(
            context,
            typedArray.getResourceId(
                R.styleable.NeumorphismClock_borderColor,
                R.color.defaultBorderColor
            )
        )
        minHourHandsColor = ContextCompat.getColor(
            context,
            typedArray.getResourceId(
                R.styleable.NeumorphismClock_minHourHandsColor,
                R.color.defaultMinHourHandsColor
            )
        )
        secondsHandColor = ContextCompat.getColor(
            context,
            typedArray.getResourceId(
                R.styleable.NeumorphismClock_secondsHandColor,
        ...
        textColor = ContextCompat.getColor(
            context,
            typedArray.getResourceId(
                R.styleable.NeumorphismClock_textColor,
                R.color.defaultTextColor
            )
        )
        textFontSize = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_PX,
            typedArray.getDimension(R.styleable.NeumorphismClock_textSize, textFontSize),
            context.resources.displayMetrics
        )
        iconColor = ContextCompat.getColor(
            context,
            typedArray.getResourceId(
                R.styleable.NeumorphismClock_iconColor,
                R.color.defaultIconColor
            )
        )
        iconSize = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_PX,
            typedArray.getDimension(R.styleable.NeumorphismClock_iconSize, iconSize),
            context.resources.displayMetrics
        )
    ...
    private fun initialize() {
        ...
        currentDay =
            "${PersianDate().dayName()} ${PersianDate().shDay} ${PersianDate().monthName()}"
    ...
    private fun drawCurrentDayText(canvas: Canvas) {
        ...
        canvas.drawText(
            currentDay,
            rectF.centerX(),
            (rectF.centerY() - defaultMargin) + (textSize / 2),
            mPaint
        )
    ...
    private fun drawNumerals(canvas: Canvas) {
        ...
        for (hour in mClockHours) {
            ...
            val y =
                ((mHeight / 2).toDouble() + sin(angle) * mRadius + (mRect.height() / 2)).toFloat()
